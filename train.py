...
for epoch in range(n_epochs):
    ...
    time_end = time.time()
    print(f'    training acuracy: {train_acc*100:.3f}% ({time_end-time_start:.3f}s)')
    train_acc_log.append(train_acc)

    time_start = time.time()
    val_acc = models.test_accuracy(model, dataloader_test)
    time_end = time.time()
    print(f'    validation acuracy: {val_acc*100:.3f}% ({time_end-time_start:.3f}s)')
